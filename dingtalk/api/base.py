...
class RestApi(object):
    ...
    def __init__(self, url=None):
        ...
        if url.find("http://") >= 0:
            ...
            raise RequestException("http protocol != validate.")
    ...
    def getResponse(
        ...
        if accessKey != "":
            ...
            if self.__path.find("?") > 0:
                fullPath = (
                    (self.__path + "&access_token=" + str(authrize))
                    if len(str(authrize)) > 0
                    else self.__path
                )
            else:
                fullPath = (
        ...
        if self.getHttpMethod() == "GET":
            ...
            connection.request(
                self.getHttpMethod(), fullPath, body=body, headers=header
            )
    ...
    def computeSignature(self, secret, canonicalString):
        ...
        return str(
            base64.b64encode(hmac.new(sec, message, digestmod=hashlib.sha256).digest())
        )
